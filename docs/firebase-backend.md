...
- **빌드:** `next.config.ts`가 빌드할 때 버전을 `APP_VERSION`으로 넣는다. 모든 페이지 아래에 `임통 vX.Y.Z`로 보인다.
- **새 버전 확인:** 열린 페이지는 `/api/version`(캐시 없음)을 확인해 **자기보다 높은** 버전이 오면 새로고침 안내를 띄운다.
  - 확인 시점: 5초 뒤, 10분마다, 탭으로 돌아올 때.
  - 롤아웃 중에 옛 인스턴스가 낮은 버전을 답해도 안내는 뜨지 않는다.
- **배포 절차:** 버전을 올리지 않고 배포하면 열린 화면에 안내가 가지 않는다. 사용자가 보는 변경을 배포할 때는 먼저 버전을 올린다. 커밋 메시지는 `release: vX.Y.Z` 형식으로 한다.
  ```
  pnpm version patch --no-git-tag-version   # 수정 (0.1.0 → 0.1.1)
  pnpm version minor --no-git-tag-version   # 기능 (0.1.0 → 0.2.0)
  git commit -am "release: vX.Y.Z" && pnpm deploy:apphosting
  ```
