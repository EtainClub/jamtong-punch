...
> 진행 현황과 앞으로 할 일은 [`roadmap.md`](./roadmap.md)에서 관리한다.
...
- **반사 게임** `/people/[slug]/play/punch|cheer`
  ...
  - 타격감: 진동, 버건디/남색 플래시, 흔들림, 파편, 세션 동안만 남는 흔적. 효과음은 기본 켜짐이고 🔊 버튼으로 끈다(2026-09-25).
  - 헤더 카드: 최신 언행과 출처.
...
- **게임 허브** `/play`, 헤더 `게임` 메뉴. 허브에는 종목 카드 네 장과 기록 규칙 설명만 둔다. 인물 목록은 `/play/reflex`로 분리해, 인물이 늘어도 다른 종목까지 스크롤하지 않게 한다.
- **무작위 월드컵 (2026-09-25 추가)**
  - 운영자 대진표 없이도 열린다.
  - `/play/worldcup/statements`: 공개된 언행 전체에서 16강을 뽑고, 모자라면 8강으로 한다.
  - `/play/worldcup/people`: 게임 대상 인물에서 뽑는다.
  - 질문은 고정 문구에서 고른다. 언행은 "더 문제인가 / 더 시급한가", 인물은 "더 펀치하고 싶은 / 더 응원하고 싶은 사람은?"이다.
  - 서버는 `auto-statements`·`auto-people` 대진의 1라운드 참가자를 공개(인물은 `playable`) 여부, 중복 없음, 8/16명 조건으로 확인하고 진행 순서를 검증한다(`lib/comparison/progression.ts`).
  - 운영자가 공개한 대진표는 그대로 `/play/worldcup/[bracket]`에서 열린다.
- **월드컵 선택 연출 (2026-09-25)**
  - 고르면 선택한 카드에 이모지가 뜨고 효과음이 난다. 0.42초 뒤 다음 대결로 넘어간다.
  - 이모지와 효과음은 질문에 따라 다르다. 더 펀치하고 싶은 사람과 더 문제인 언행은 👊와 펀치 소리, 더 응원하고 싶은 사람은 ❤️와 차임, 더 시급한 언행은 ⚡와 짧은 선택음이다.
  - 결승에서 1위를 고르면 🏆와 팡파르가 나온다.
- **인물 월드컵 결정 (2026-09-25, 운영자)**
  - implementation-design 11장 7번은 실명 인물의 순위·토너먼트를 만들지 않는다고 했지만, **운영자 결정으로 인물 월드컵을 연다.**
  - 완충은 셋이다. 결과는 본인의 비교 원장에만 남긴다. 인물 순위나 집계 화면은 만들지 않는다. 결과 화면에 "임통은 이 결과로 순위를 공개하지 않는다"고 적는다.
  - 인물 월드컵 화면은 검색에서 뺀다.
- **기록 규칙 문구**
  - 대상마다 하루 1건이고, 같은 날 다시 하면 바뀔 뿐 늘지 않는다. 대상이 다르면 따로 남는다. 점수는 무게가 없다. 월드컵은 입장이 아니다.
  - 허브(`PlayRules`), 게임 시작 화면, 인물 페이지에 같은 말로 적는다.
