# 임통 — 인물 아카이브 전환 설계 (rev. 6)

- 상태: 설계 확정 — Phase A~C 구현, D 일부 구현, 사용자 참여 등록 구현 (2026-09-24)
- 작성일: 2026-09-23
...
### rev. 6 — 사용자 참여 등록 (2026-09-24)

| 항목 | rev. 5 | rev. 6 | 이유 |
| --- | --- | --- | --- |
| 등록 주체 | 운영자만 | **구글 로그인 사용자 누구나** 인물·언행·시선·출처·쟁점을 등록. 사건·월드컵은 운영자만 | 운영자 혼자서는 기록이 쌓이지 않는다 |
| 공개 | 운영자가 저장하면 공개 | **검토 후 공개.** 기여자는 초안·검토 요청까지, 공개·반려·보관은 운영자 | 공개하면 체인에 지문이 올라 되돌릴 수 없다. 체인에 오르는 것은 운영자가 본 것뿐이어야 한다 |
| 표시 | 없음 | 공개 카드에 **"등록: 닉네임"**. 구글 이름·이메일은 쓰지 않는다 | 기여를 드러내되 신원은 노출하지 않는다 |
| 반려 | 없음 | 운영자가 **사유와 함께 반려**, 기여자가 고쳐 다시 요청 | 고칠 기회 없이 버리면 기여가 끊긴다 |
...
### 5.1d 사용자 참여 등록 (2026-09-24, rev. 6)

**역할.** 역할은 ID 토큰으로 가른다. 익명 ID는 펀치·응원만 할 수 있다. 구글 계정(`sign_in_provider`가 익명·커스텀이 아닌 것)은 **기여자**다. `ops` 클레임이 있으면 **운영자**다. 기여자는 닉네임(2~20자, `contributors/{uid}`)을 정해야 저장할 수 있다.

**상태.** `draft` → `review` → (`rejected` ↔ `review`) → `published` → `archived`

| 누가 | 할 수 있는 것 |
| --- | --- |
| 기여자 | 인물·언행·시선·출처·쟁점을 `draft`·`review`로 저장. 자기 것이 `draft`·`review`·`rejected`인 동안만 고치거나 지운다. 사진과 `playable`은 넣을 수 없다. 공개 기록이 인용한 출처는 고칠 수 없다(주소가 지문에 들어간다) |
| 운영자 | 모든 상태 전환. `rejected`는 저장으로 만들 수 없고 반려(`PATCH { rejectNote }`, 1~500자)로만 만든다. 반려할 수 있는 것은 공개 전 기록뿐이다. 공개 기록은 보관으로 내린다 |

- **메타 필드**(`authored()`가 벗긴다. 지문에 들어가지 않는다):
  - `contributor { uid, nickname }`: 처음 등록한 기여자. 운영자가 고치거나 공개해도 남는다.
  - `rejection { note, by, at }`: 다시 요청해도 다음 검토를 위해 남고, 공개하면 지운다.
- **목록:** 운영자는 모든 기록을 본다. 기여자는 공개 기록(참조용)과 자기 기록만 본다.
- **배지:** 헤더의 `운영` 배지는 검토 대기 수(`status == review` count 집계)다. `등록하기` 배지는 내 반려 수다.
- **남용 방지:** 기여자는 하루 50회까지 저장한다(`contributors/{uid}/daily/{KST 날짜}`).
- **중복 경고:** 저장 전에 편집기가 불러온 목록과 비교해 경고한다. 막지는 않는다. 같은 영상에 다른 발언이 있을 수 있다.
  - 출처: 같은 주소. 유튜브는 영상 ID로 비교하고, 추적 파라미터는 뺀다.
  - 인물: 같은 이름이나 별칭.
...
9. ~~게임 화면~~: **구현(2026-09-25).** 반사 게임(얼굴 타격), 훑어보기, 월드컵을 만들었다. 얼굴 타격은 11장 2번 검토 전에 운영자가 위험을 감수하고 열었다(10장 Phase D).
...
### Phase D — 게임 재연결과 언행 호불호 — 구현됨 (화면 2026-09-25)

**게임 화면 (2026-09-25):**
- **반사 게임** `/people/[slug]/play/punch|cheer`
  - 권리 확인된 원본 사진이 3×3 판을 옮겨 다닌다. 1000ms에서 시작해 맞힐 때마다 20ms씩 빨라지고, 400ms가 하한이다.
  - 목숨과 시간 제한이 없다. 끝내기를 누르거나 20초 동안 치지 않으면 끝난다.
  - 타격감: 진동, 버건디/남색 플래시, 흔들림, 파편, 세션 동안만 남는 흔적. 효과음은 기본 꺼짐이다.
  - 헤더 카드: 최신 언행과 출처.
  - 기록: 끝날 때 `game: "reflex"`로 입장 1건. 한 번도 맞히지 않으면 기록하지 않는다.
  - 결과 화면: 개인 점수와 기록된 입장을 따로 보여 준다.
- **훑어보기** `/play/swipe`
  - 게임 대상 인물과 최근 공개 언행을 섞어 최대 25장.
  - 왼쪽 펀치, 오른쪽 응원, 위 잘 모름. 버튼과 방향키로도 고른다.
  - 되돌리기 1회. 위의 3장만 렌더한다.
  - 기록: 끝날 때 카드별 `dwellMs`와 함께 한 번에 제출한다.
- **언행 월드컵** `/play/worldcup/[bracket]`
  - 운영자가 정한 대진 순서대로 둘 중 하나를 고른다.
  - 비교 원장(`/api/comparison`)에만 쓰고 입장 수치에는 넣지 않는다.
  - 결과 화면에 "지지율이 아니다"라고 적는다. 공유 링크는 있고 공유 이미지는 아직 없다.
- **게임 허브** `/play`, 헤더 `게임` 메뉴.
- 게임 화면은 검색에서 뺀다(`noindex`).

**얼굴 타격 결정 (2026-09-25, 운영자):**
- implementation-design 11장 2번(초상권) 검토가 닫히기 전이지만 **얼굴 타격을 포함해 연다.** 초상권·인격권 민사 위험을 운영자가 감수한 결정이다.
- 완충은 설계대로 둔다. 타격 화면은 저장·공유·서버 업로드가 없고, 공유 이미지(OG)에는 얼굴을 넣지 않는다. OG 인물 카드에서도 사진을 뺐다.
- 삭제·교체 요구가 오면 해당 인물의 `playable`을 끄는 것이 1차 대응이다. 사진과 기록은 남는다.
