  ...
  return <>
    ...
    <main className={styles.wideShell}>
      ...
      {people.length ? <ul className={styles.peopleGrid}>{people.map((person) => <li key={person.id}>
        <Link className={styles.personTile} href={`/people/${person.id}`}><Avatar person={person} size={52} /><div><strong>{person.name}</strong><span>{currentRole(person)}</span><span>기록 {person.counts.statements} · 평가 {person.counts.evaluationsReceived} · 관계 {person.counts.relations}</span></div></Link>
