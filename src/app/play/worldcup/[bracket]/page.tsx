...
export default async function BracketPage({ params }: Props) {
  ...
  return <>
    ...
    <main className={styles.shell}>
      <WorldCup bracketId={bracket.id} question={QUESTIONS[bracket.questionId]} contenders={statements.map((statement) => statementContender(statement, names, sources))} sharePath={`/play/worldcup/${bracket.id}`} />
    </main>
  </>;
}
