      ...
      <section className={styles.editor}>
        ...
        <div className={styles.form}>
          ...
          {type === "subjects" && <>
            ...
            <fieldset className={styles.wide}><legend>이미지와 권리 <b className={styles.required}>필수</b></legend><div className={styles.imageArea}><input className={styles.visuallyHidden} ref={fileRef} type="file" accept="image/jpeg,image/png,image/webp" onChange={(event) => { const file = event.target.files?.[0]; if (file) void uploadImage(file); event.target.value = ""; }} /><div className={styles.preview} style={stringValue(image.path) ? { backgroundImage: `url(${stringValue(image.path)})` } : undefined}>{stringValue(image.path) ? "" : "이미지 미리보기"}</div><div><strong>사진을 선택하거나 화면에서 붙여넣으세요</strong><p>JPG · PNG · WebP, 최대 5MiB</p><button className={styles.secondary} onClick={() => fileRef.current?.click()} disabled={uploading} type="button">{uploading ? "업로드 중…" : "파일 선택"}</button></div></div><div className={styles.fieldGrid}><Field label="원본 출처 URL" required hint="업로드한 파일의 원 출처를 기록하세요"><input type="url" value={stringValue(image.sourceUrl)} onChange={(event) => setImageField("sourceUrl", event.target.value)} placeholder="https://" required /></Field><Field label="라이선스" required><select value={stringValue(image.license)} onChange={(event) => setImageField("license", event.target.value)}><option value="public">공개</option><option value="cleared">권리 확인</option><option value="link-only">링크만</option></select></Field><Field label="권리 상태" required><select value={stringValue(image.rightsStatus)} onChange={(event) => setImageField("rightsStatus", event.target.value)}><option value="pending">확인 대기</option><option value="cleared">확인됨</option><option value="replace-requested">교체 요청</option></select></Field></div></fieldset>
          </>}
